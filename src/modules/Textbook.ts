...
export class Textbook {
    ...
    // Grabs a textbook
    static async get(ISBN: string, UseCache: boolean = true) {
        // Logging
        DevExecute(log.warn, `Attempting to grab textbook (${ISBN})`)
        
        // Attempt to get it from the cache
        const CachedBook = TextbookCache.find(textbook => textbook.ISBN == ISBN)
        if (CachedBook && UseCache) {
            DevExecute(log.info, `Retrieved textbook (${ISBN}) from cache`)
            return CachedBook
        }
            

        // Query the database
        const [result] = await Database.Connection.query<ITextbookRow[]>("SELECT * FROM `textbook` WHERE `ISBN`=?", [ISBN])

        // Check we got a result
        const TextbookDB = result[0]
        if (!TextbookDB) {
            DevExecute(log.error, `Textbook (${ISBN}) was not found in database`)
            return
        }
        ...
        DevExecute(log.info, `Added textbook (${ISBN}) to cache`)
        return textbook
    }
    ...
    static async refresh() {
        ...
        for (const TextbookDB of result) {
            ...
            DevExecute(log.info, `Added textbook (${TextbookDB.ISBN}) to cache`)
            TextbookCache.push(
    ...
    static async add(Data: ITextbook, ModifyCache: boolean = true) {
        // Logging
        DevExecute(log.warn, `Attempting to add textbook (${Data.ISBN}) to database${ModifyCache ? " and cache" : ""}`)
        ...
        if (await Textbook.get(Data.ISBN)) {
            ...
            // Output
            const Message = `Textbook (${Data.ISBN}) was already within the database`
            DevExecute(log.error, Message)
            throw(new Error(Message))
        } 

        // Add it to the database
        await Database.Connection.query("INSERT INTO `textbook` (`Subject`, `Title`, `ISBN`) VALUES (?, ?, ?)", [Data.Subject, Data.Title, Data.ISBN])
        DevExecute(log.info, `Added textbook (${Data.ISBN}) to database`)
        ...
        // Add it
        TextbookCache.push(textbook)
        DevExecute(log.info, `Added textbook (${Data.ISBN}) to cache`)
    }
    ...
    static async remove(Data: ITextbook, ModifyCache: boolean = true) {
        // Logging
        DevExecute(log.warn, `Attempting to remove textbook (${Data.ISBN}) from database${ModifyCache ? " and cache" : ""}`)

        // Make sure it already exists
        if (!await Textbook.get(Data.ISBN)) {
            const Message = `Textbook (${Data.ISBN}) does not exist within database`
            DevExecute(log.error, Message)
            throw(new Error(Message))
        }
        ...
        await Database.Connection.query("DELETE FROM `textbook` WHERE `ISBN`=?", [Data.ISBN])
        DevExecute(log.info, `Removed textbook (${Data.ISBN}) from database`)
        ...
        DevExecute(log.info, `Removed textbook (${Data.ISBN}) from cache`)
    }
